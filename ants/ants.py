...
class Place:
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if insect.is_queen: # check if it is a true queen
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while location!=hive:
            if self.in_range(rg):
                if len(location.bees)!=0:
                    return random_or_none(location.bees)
            location=location.entrance
            rg+=1
        return None

    def in_range(self,rg):
        return rg in range(self.min_range,self.max_range+1)
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, colony):
        ...
        """
        if self.truequeen: # check if it is true queen or imposters
            ScubaThrower.action(self,colony)
        # Apply double damage
            new_exit,new_entrance=self.place,self.place
            while new_exit!=None: # iterate all the exit
                self.apply_double(new_exit.ant)
                new_exit=new_exit.exit
            while new_entrance!=None: # iterate all the entrance
                self.apply_double(new_entrance.ant)
                new_entrance=new_entrance.entrance
            colony.queen=QueenPlace(self.place,colony.queen)
        else:
             self.reduce_armor(self.armor)
    def double(self,ant): # double the damage
        if ant.is_queen is False and ant.is_doubled is False:
            ant.damage*=2
            ant.is_doubled=True
                
    def apply_double(self,ant): # apply dbuble damage on ant and contained ant
        if ant!=None:
            self.double(ant)
            if ant.container:
                self.double(ant.ant)
