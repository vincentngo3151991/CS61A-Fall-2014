...
def extract_words(text):
    ...
    ['cat', 'on', 'my', 'keyboard']
    """
    new_string='' # define new empty string
    k=0
    while k<len(text):
        if text[k] in ascii_letters: # if text[k] is in ascii_letters, leaves it alone
            new_string+=text[k]
        else:
            new_string +=' ' # if text[k] is not in ascii_letters, then replaces it with space
        k+=1
...
def analyze_tweet_sentiment(tweet):
    ...
    False
    """
    total,counter,words=0,0,tweet_words(tweet) # define total value and the number of sentiment word.
    for i in range(0,len(words)):
        if has_sentiment(get_word_sentiment(words[i])):
            total+=sentiment_value(get_word_sentiment(words[i]))
            counter+=1
    if counter!=0:
        return make_sentiment(total/counter)
...
def find_centroid(polygon):
    ...
    for i in range(0,len(polygon)-1):
        part=x(polygon[i])*y(polygon[i+1])-x(polygon[i+1])*y(polygon[i])
        A += 0.5*part
        lat+=(x(polygon[i])+x(polygon[i+1]))*part
        lon+=(y(polygon[i])+y(polygon[i+1]))*part
    if A!=0:
        return [lat/(6*A), lon/(6*A),abs(A)]
    else:
        return [x(polygon[0]),y(polygon[0]),0]
def find_state_center(polygons):
    ...
    for i in range(0,len(polygons)):
        cx=find_centroid(polygons[i])[0]
        cy=find_centroid(polygons[i])[1]
        Ai=find_centroid(polygons[i])[2]
        cx_A+=Ai*cx
        cy_A+=Ai*cy
        A+=Ai
    return make_position(cx_A/A,cy_A/A)
...
def list_of_state_center(us_state):
    """ return a list of state center
    """
    state_coord=list(us_state.values())
    return [find_state_center(elem) for elem in state_coord]
...
def average_sentiments(tweets_by_state):
    ...
    """
    lst=[] # create a blank list
    for state in tweets_by_state.items(): # check each state individually, state=['state',tweet]
        counter,score=0,0
        for tweet in state[1]: # check each tweet. If tweet has sentiment, add the score and counter
            if has_sentiment(analyze_tweet_sentiment(tweet)):
                score+=sentiment_value(analyze_tweet_sentiment(tweet))
                counter+=1
        if counter!=0:
           lst+=[[state[0],score/counter]]
    return dict(lst)
