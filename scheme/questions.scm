...
(define (list-partitions total max-pieces max-value)
  (define part (partition total max-value))
  (define (smaller? s) (<= (len s) max-pieces))
  (keep-if smaller? part))
(define (partition val size)
  (cond 
   ((or (< val 0) (= size 0)) nil)
   ((= val 0) (cons nil nil))
   (else
    (define yes (partition (- val size) size))
    (define no (partition val (- size 1)))
    (define (add s) (cons size s))
    (define include (apply-to-all add yes))
    (append include no))))
(define (len s)
  (if (null? s) 
      0
  ...
  (cond 
        ...
        ((let? expr)
         (let ((values (cadr expr))
           ...
           (cons (cons 'lambda (cons (car (zip values)) (apply-to-all analyze body))) (cadr (zip values))
           )))
        (else
