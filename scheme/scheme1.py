...
def scheme_apply(procedure, args, env):
    ...
    if isinstance(procedure, PrimitiveProcedure):
        ...
        "*** YOUR CODE HERE ***"
        for f in procedure.formals:
            if f not in env.bindings.items():
                env.bindings[f] = args[0]
...
class Frame:
    ...
    def make_call_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError("too many or too few arguments in lists")
        return frame
...
def do_define_form(vals, env):
    ...
    if scheme_symbolp(target):
        ...
        "*** YOUR CODE HERE ***"
        if isinstance(target.first, int):
            raise SchemeError("bad argument to define")
        else:
            lambda_args = Pair(target.second, vals.second)
            lambda_result = do_lambda_form(lambda_args, env)
            env.define(target.first, lambda_result)
            return target.first
    else:
        raise SchemeError("bad argument to define")
...
def do_let_form(vals, env):
    ...
    "*** YOUR CODE HERE ***"
    for b in bindings:
        if len(b) % 2 == 0 and scheme_symbolp(b[0]):
            names = Pair(b[0], names)
            values = Pair(b[1], values)
        else:
            raise SchemeError("too many arguments/arguments are not symbols")
    new_env = env.make_call_frame(names, values)

    for k, v in new_env.bindings.items():
        if scheme_symbolp(v):
            new_env.bindings[k] = new_env.parent.lookup(v)
        else:
            new_env.bindings[k] = scheme_eval(v, new_env.parent)
...
def do_begin_form(vals, env):
    ...
    "*** YOUR CODE HERE ***"
    while vals != nil:
        if isinstance(vals.first, int) or 'define' not in vals.first:
            final = scheme_eval(vals.first, env)
            result = vals.first
        else:
...
def check_formals(formals):
    ...
    """
    "*** YOUR CODE HERE ***"
    symbols_list = [f for f in formals]
    for s in symbols_list:
        if symbols_list.count(s) > 1:
...
def scheme_optimized_eval(expr, env):
    ...
    while True:
        ...
        if (scheme_symbolp(first) # first might be unhashable
            ...
            # Similar to scheme_apply with tail call
